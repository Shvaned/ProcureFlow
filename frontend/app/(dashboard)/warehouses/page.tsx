...
export default function WarehousesPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowCreate(false);
      setForm({ code: "", name: "", city: "", country: "", warehouse_type: "Distribution" });
      fetchWarehouses();
    } catch (err: any) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage warehouses, zones, and bins</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={fetchWarehouses} disabled={loading}>
            <RefreshCw className={`mr-2 h-4 w-4 ${loading ? "animate-spin" : ""}`} /> Refresh
          </Button>
          <Button onClick={() => setShowCreate(!showCreate)}>
            <Plus className="mr-2 h-4 w-4" /> {showCreate ? "Cancel" : "Add Warehouse"}
          </Button>
        </div>
      </div>
      ...
      {loading ? (
        <div className="py-16 text-center text-muted-foreground">Loading warehouses...</div>
      ) : warehouses.length === 0 ? (
        <div className="py-16 text-center text-muted-foreground">
          No warehouses. Create your first warehouse above.
        </div>
      ) : (
        warehouses.map((wh) => (
          <Card key={wh.id}>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs font-medium ${wh.is_active ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-muted text-muted-foreground"}`}
                  >
                    {wh.is_active ? "Active" : "Inactive"}
                  </span>
                  <Button variant="ghost" size="sm" onClick={() => loadZones(wh.id)}>
                    Zones
                  </Button>
                </div>
              </div>
