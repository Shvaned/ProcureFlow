...
export default function SuppliersPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await apiClient.post("/api/v1/suppliers", form);
      setShowCreate(false);
      setForm({ code: "", legal_name: "", email: "", phone: "", country: "India" });
      fetchSuppliers(page);
    } catch (err: any) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            {total} suppliers · rating, performance, contact tracking
          </p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => fetchSuppliers(page)} disabled={loading}>
            <RefreshCw className={`mr-2 h-4 w-4 ${loading ? "animate-spin" : ""}`} /> Refresh
          </Button>
          <Button onClick={() => setShowCreate(!showCreate)}>
            <Plus className="mr-2 h-4 w-4" /> {showCreate ? "Cancel" : "Add Supplier"}
          </Button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="py-16 text-center text-muted-foreground">
          No suppliers. Add your first supplier above.
        </div>
      ) : (
        <Card>
                  ...
                  .map((s) => (
                    <>
                      <tr key={s.id} className="border-b hover:bg-muted/50">
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={`rounded-full px-2 py-0.5 text-xs font-medium ${s.is_active ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-muted text-muted-foreground"}`}
                          >
                            ...
                            <div className="grid gap-4 md:grid-cols-3">
                              ...
                              <div>
                                ...
                                <div className="text-xs space-y-1">
                                  <div>GST: {detail.gst_number || "--"}</div>
                                  <div>PAN: {detail.pan || "--"}</div>
                                  <div>Payment Terms: {detail.payment_terms || "--"}</div>
                                  <div>Currency: {detail.currency}</div>
                                  <div>Preferred: {detail.is_preferred ? "Yes" : "No"}</div>
                                </div>
                              </div>
                              <div>
