...
interface Role {
  ...
  is_system: boolean;
  permissions: { name: string; group: string }[];
}
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            {roles.length} roles · 49 permissions across 12 groups
          </p>
        </div>
            ...
            >
              <CardContent className="py-6">
                <div className="flex items-start justify-between mb-3">
                  ...
                  {r.is_system && (
                    <span className="rounded-full bg-blue-50 dark:bg-blue-950/20 px-2 py-0.5 text-xs text-blue-700 dark:text-blue-400">
                      System
                    </span>
                  )}
                </div>
                <div className="flex flex-wrap gap-1">
                  {r.permissions?.slice(0, expanded === r.id ? 99 : 5).map((p) => (
                    <span
                      key={p.name}
                      className="rounded-full bg-muted px-1.5 py-0.5 text-xs font-mono"
                    >
                      {p.name}
                    </span>
                  ))}
                  {(r.permissions?.length || 0) > 5 && expanded !== r.id && (
                ...
                {expanded === r.id && (
                  <div className="mt-3 pt-3 border-t">
                    <p className="text-xs font-medium mb-1">
                      All {r.permissions?.length || 0} Permissions
                    </p>
