...
export default function ProcurementAnalytics() {
  ...
  useEffect(() => {
    apiClient
      .get<{ data: ProcData }>("/api/v1/analytics/procurement")
      .then((r) => {
        setData(r.data);
        setLoading(false);
      })
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">PO trends, approval analysis, spend breakdown</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          <CardContent className="py-4">
            ...
            <p className="text-2xl font-bold">{data?.total_purchase_orders || 0}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="py-4">
            <p className="text-xs uppercase text-muted-foreground">Total Spend</p>
            <p className="text-2xl font-bold">₹{totalSpend}L</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="py-4">
            <p className="text-xs uppercase text-muted-foreground">Draft POs</p>
            <p className="text-2xl font-bold">
              {chartData.find((s) => s.name === "draft")?.count || 0}
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="py-4">
            <p className="text-xs uppercase text-muted-foreground">Approved POs</p>
            <p className="text-2xl font-bold">
              {chartData.find((s) => s.name === "approved")?.count || 0}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>Spend by Status (₹K)</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle>PO Count by Status</CardTitle>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={chartData}>
