    ...
    <div className="space-y-6">
        ...
        <Card className="border-red-500/30">
          <CardContent className="flex items-center justify-between py-4">
            <div>
              ...
              <p className="text-2xl font-bold text-red-500">{expiring.length}</p>
            </div>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="py-16 text-center text-muted-foreground">
              No inventory records found. Create products and warehouses first.
            </div>
          ) : (
              ...
              <tbody>
                {items
                  .filter(
                    (i) =>
                      search === "" ||
                      i.product_id.includes(search) ||
                      i.lot_number?.includes(search)
                  )
                  .map((i) => (
                    <tr key={i.id} className="border-b hover:bg-muted/50">
                      ...
                      <td className="py-3 px-4">
                        <Button variant="ghost" size="sm" onClick={() => viewTxns(i.id)}>
                          History
                        </Button>
                      </td>
                    </tr>
                ...
                <tbody>
                  {selectedTxn.txns.map((t, i) => (
                    <tr key={i} className="border-b">
                      ...
                      <td className="py-2">{t.before}</td>
                      <td className="py-2">{t.after}</td>
                      <td
                        className={`py-2 font-medium ${t.change < 0 ? "text-red-500" : "text-green-500"}`}
                      >
                        {t.change > 0 ? "+" : ""}
                        {t.change}
                      </td>
      ...
      {alerts.length > 0 && (
        <Card className="border-orange-500/20">
          <CardHeader>
            <CardTitle className="text-base">Low Stock Alerts ({alerts.length})</CardTitle>
          </CardHeader>
          <CardContent>
            <table className="w-full text-sm">
              <thead>
              ...
              <tbody>
                {alerts.slice(0, 10).map((a) => (
                  <tr key={a.id} className="border-b">
                    <td className="py-2 font-mono text-xs">{a.product_id.slice(0, 8)}...</td>
                    <td className="py-2 text-right text-orange-500">{a.available}</td>
                    <td className="py-2 text-right">{a.reorder_level}</td>
                  </tr>
                ))}
              </tbody>
            </table>
