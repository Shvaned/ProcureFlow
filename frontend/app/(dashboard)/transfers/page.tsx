    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Inter-warehouse stock transfers with audit trail</p>
        </div>
        <Button onClick={() => setShowCreate(!showCreate)}>
          <Plus className="mr-2 h-4 w-4" /> {showCreate ? "Cancel" : "New Transfer"}
        </Button>
      </div>
      ...
      {success && (
        <div className="rounded-md bg-green-100 dark:bg-green-900/20 p-4 text-sm text-green-700 dark:text-green-400">
          {success}
        </div>
      )}
      ...
      {showCreate && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Create Stock Transfer</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <div>
                ...
                {form.items.map((item, idx) => (
                  <div key={idx} className="mb-2 flex gap-2">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const items = [...form.items];
                        items[idx].inventory_id = e.target.value;
                        setForm({ ...form, items });
                      }}
                    />
                    <input
                ...
                <Button
                  ...
                  onClick={() =>
                    setForm({
                      ...form,
                      items: [...form.items, { product_id: "", inventory_id: "", quantity: 1 }],
                    })
                  }
                >
                  + Add Item
      ...
      <Card>
        <CardContent className="py-16 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Create an inter-warehouse transfer above. Each transfer records a full audit trail.
          </p>
        </CardContent>
      </Card>
    </div>
