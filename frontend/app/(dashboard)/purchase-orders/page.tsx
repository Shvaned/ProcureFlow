...
interface PODetail {
  ...
  approvals: { status: string; notes: string | null; decided_at: string | null }[];
}
...
export default function PurchaseOrdersPage() {
  ...
  const approvePO = async (poId: string) => {
    try {
      await apiClient.post(`/api/v1/purchase-orders/${poId}/approve`, {});
      fetchPOs(page);
      setDetail(null);
    } catch (err: any) {
      setError(err.message);
  ...
  const statusBadge = (s: string) => {
    ...
    return (
      <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${colors[s] || "bg-muted"}`}>
        {s}
      </span>
    );
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">{total} POs · draft, approved, received tracking</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => fetchPOs(page)} disabled={loading}>
          ...
          >
            <Plus className="mr-2 h-4 w-4" /> {showCreate ? "Cancel" : "New PO"}
          </Button>
        </div>
      </div>
              ...
              {form.items.map((item, idx) => (
                <div key={idx} className="flex gap-2">
                  ...
                  <input
                    type="number"
                    step="0.01"
                    className="w-28 rounded-md border px-3 py-1 text-sm"
      ...
      {loading ? (
        ...
        <div className="py-16 text-center text-muted-foreground">
          No purchase orders. Create your first PO above.
        </div>
      ) : (
        <Card>
          <CardContent className="p-0">
            <table className="w-full text-sm">
              ...
              <tbody>
                {pos
                  .filter(
                    (p) => search === "" || p.po_number.toLowerCase().includes(search.toLowerCase())
                  )
                  .map((p) => (
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-base">
                {detail.po_number} · {statusBadge(detail.status)} · ₹
                {detail.total_amount?.toLocaleString()}
              </CardTitle>
              <div className="flex gap-2">
                {detail.status === "draft" && (
                  <Button size="sm" onClick={() => approvePO(detail.id)}>
              ...
              <tbody>
                {detail.items.map((item, i) => (
                  <tr key={i} className="border-b">
                    <td className="py-2 font-mono text-xs">{item.product_id.slice(0, 12)}...</td>
                    <td className="py-2 text-right">{item.quantity}</td>
                    <td className="py-2 text-right">{item.received_quantity}</td>
                    <td className="py-2 text-right">₹{item.unit_cost?.toLocaleString()}</td>
              ...
              <div className="mt-4">
                ...
                {detail.approvals.map((a, i) => (
                  <div key={i} className="text-xs text-muted-foreground">
                    {a.status} · {a.notes || "No notes"} ·{" "}
                    {a.decided_at ? new Date(a.decided_at).toLocaleString() : "Pending"}
                  </div>
                ))}
              </div>
            )}
