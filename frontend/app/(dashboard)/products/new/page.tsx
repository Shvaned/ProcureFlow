...
export default function NewProductPage() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const r = await apiClient.post<{ data: { sku: string } }>("/api/v1/sku/preview", {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setError("");
    if (!category || !brand) {
      setError("Please select a Category and Brand");
      setSaving(false);
      return;
    }
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Smart SKU generation with live preview</p>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <div>
                ...
                >
                  ...
                  <optgroup label="Lengths">
                    {LENGTHS.filter((s) => s).map((s) => (
                      <option key={s}>{s}</option>
                    ))}
                  </optgroup>
                </select>
            ...
            {previewSku && (
              <div className="rounded-lg border-2 border-primary/30 bg-primary/5 p-4">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Category: {category} · Brand: {brand} {variant ? `· Variant: ${variant}` : ""}
                </p>
              </div>
            )}
            ...
            <div>
              ...
              <input
                ...
                placeholder={previewSku || "Auto-generated or enter manually"}
              />
              <p className="text-xs text-muted-foreground mt-1">
                Leave blank to use auto-generated SKU. Admins may override.
              </p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                required
                className="w-full rounded-md border px-3 py-2 text-sm"
                placeholder="e.g., Filtek Z350 XT Composite A2"
              />
            </div>
