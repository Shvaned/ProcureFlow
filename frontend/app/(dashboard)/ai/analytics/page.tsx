...
export default function AnalyticsCopilot() {
  ...
  const ask = async (question: string) => {
    ...
    setLoading(true);
    try {
      const r = await apiClient.post<{ data: QueryResult }>("/api/v1/ai/analytics/query", {
        question,
      });
  ...
  const exportCSV = (r: QueryResult) => {
    ...
    const csv = [
      r.columns.join(","),
      ...r.rows.map((row) => r.columns.map((c) => JSON.stringify(row[c] ?? "")).join(",")),
    ].join("\n");
      ...
      return (
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={data}>
            ...
            <XAxis dataKey={labelCol} style={{ fontSize: 11 }} />
            <YAxis style={{ fontSize: 11 }} />
            <Tooltip />
            <Bar dataKey={valCol} fill="#3b82f6" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      );
    if (ct === "pie")
        ...
        <ResponsiveContainer width="100%" height={280}>
          <PieChart>
            <Pie
              ...
              dataKey={valCol}
              nameKey={labelCol}
              label={({ name, value }) => `${name?.toString().slice(0, 10)}: ${value}`}
            >
              <Tooltip />
              {data.map((_, i) => (
      ...
      return (
        <ResponsiveContainer width="100%" height={280}>
          <LineChart data={data}>
            ...
            <YAxis style={{ fontSize: 11 }} />
            <Tooltip />
            <Line type="monotone" dataKey={valCol} stroke="#3b82f6" strokeWidth={2} />
          </LineChart>
        </ResponsiveContainer>
      );
    return null;
  ...
  return (
    <div className="flex h-[calc(100vh-6rem)] flex-col">
      <div className="mb-4 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold">Analytics Copilot</h1>
          <p className="text-xs text-muted-foreground">
            Natural language → SQL → Charts. Ask business questions, get answers.
          </p>
        </div>
      </div>
        ...
        {results.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-muted-foreground max-w-md mb-6">
              Ask questions in plain English. I generate safe SQL, execute it against your ERP, and
            ...
            <div className="flex flex-wrap justify-center gap-2 max-w-xl">
              {SUPPORTED.map((q, i) => (
                <button
                  key={i}
                  onClick={() => ask(q)}
                  className="rounded-full border px-4 py-2 text-sm hover:bg-muted hover:border-primary/50 transition-colors"
                >
                  {q}
                </button>
              ))}
            ...
            <Card className={r.sql_valid ? "border-green-500/20" : "border-red-500/20"}>
              <CardContent className="py-4 space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2 text-xs">
                  <span
                    className={`flex items-center gap-1 rounded-full px-2 py-1 ${r.sql_valid ? "bg-green-100 text-green-700 dark:bg-green-900/20 dark:text-green-400" : "bg-red-100 text-red-700 dark:bg-red-900/20 dark:text-red-400"}`}
                  >
                    {r.sql_valid ? (
                ...
                <div>
                  <button
                    onClick={() => setExpandedSQL({ ...expandedSQL, [idx]: !expandedSQL[idx] })}
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div className="flex gap-1">
                        {["bar", "pie", "line", "table"].map((ct) => (
                          <button
                            key={ct}
                            onClick={() => setChartView({ ...chartView, [idx]: ct })}
                            className={`rounded px-2 py-0.5 text-xs ${(chartView[idx] || r.chart_type) === ct ? "bg-primary text-primary-foreground" : "bg-muted hover:bg-muted/80"}`}
                          >
                            {ct}
                          </button>
                        ))}
                      ...
                      <div className="overflow-x-auto rounded-md border">
                          ...
                          <thead>
                            <tr>
                              {r.columns.map((c) => (
                                <th
                                  key={c}
                                  className="bg-muted px-3 py-2 text-left text-xs font-medium"
                                >
                                  {c}
                                </th>
                              ))}
                        ...
                        {r.row_count > 50 && (
                          <div className="px-3 py-2 text-xs text-muted-foreground border-t">
                            Showing 50 of {r.row_count} rows
                          </div>
                        )}
                      </div>
                    ) : (
                  ...
                  <div className="flex flex-wrap gap-1">
                    {r.suggested_followups.map((q, i) => (
                      <button
                        key={i}
                        onClick={() => ask(q)}
                        className="rounded-full border px-3 py-1 text-xs hover:bg-muted hover:border-primary/50 transition-colors"
                      >
                        {q}
                      </button>
                    ))}
      ...
      <div className="border-t pt-4">
        <div className="flex gap-2">
          <input
            ...
            placeholder="Ask: Show inventory value by warehouse..."
            value={input}
