...
export default function AutomationStudio() {
  const [tab, setTab] = useState<"workflows" | "templates" | "history" | "analytics" | "builder">(
    "workflows"
  );
  ...
  const handleCreate = async () => {
    ...
    setForm({ name: "", description: "", trigger_config: "" });
    fetchAll();
  };

  const handlePublish = async (id: string) => {
    await apiClient.post(`/api/v1/workflows/${id}/publish`);
    fetchAll();
  };
  const handleExecute = async (id: string) => {
    await apiClient.post(`/api/v1/workflows/${id}/execute`);
    fetchAll();
  };
  ...
  const handleAiGenerate = async () => {
    ...
    setAiLoading(true);
    try {
      const r = await apiClient.post<{ data: { workflow_description: string } }>(
        "/api/v1/ai/automation/workflows/generate",
        { description: aiInput }
      );
  ...
  const statusBadge = (s: string) => {
    ...
    return (
      <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${colors[s] || ""}`}>{s}</span>
    );
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold">Automation Studio</h1>
            <p className="text-xs text-muted-foreground">Visual workflow automation platform</p>
          </div>
        </div>
        ...
        <>
            ...
            <div className="space-y-4">
              <Card>
                ...
                <CardContent>
                  <div className="flex gap-2">
                    ...
                    <input
                      className="flex-1 rounded-md border px-3 py-2 text-sm"
                      placeholder="Trigger (e.g., inventory_below_safety_stock)"
              ...
              {workflows.length === 0 ? (
                <div className="py-16 text-center text-muted-foreground">
                  No workflows yet. Create one above or use a template.
                </div>
              ) : (
                    ...
                    <Card key={w.id} className="hover:shadow-sm transition-shadow">
                      <CardContent className="flex items-center justify-between py-4">
                        ...
                        <div className="flex gap-1">
                          {w.status === "draft" && (
                            <Button size="sm" variant="outline" onClick={() => handlePublish(w.id)}>
                              <Play className="mr-1 h-3 w-3" /> Publish
                            </Button>
                          )}
                          {w.status === "published" && (
                            <Button size="sm" variant="outline" onClick={() => handleExecute(w.id)}>
                              <Zap className="mr-1 h-3 w-3" /> Execute
                            </Button>
                          )}
                  ...
                  <CardContent className="py-6">
                    ...
                    <div className="mt-3 flex flex-wrap gap-1">
                      ...
                      {t.actions.slice(0, 2).map((a, j) => (
                        <span key={j} className="rounded-full bg-muted px-2 py-0.5 text-xs">
                          {a}
                        </span>
                      ))}
                      {t.actions.length > 2 && (
                        <span className="text-xs text-muted-foreground">
                          +{t.actions.length - 2} more
                        </span>
                      )}
                    </div>
                    <p className="text-xs text-primary mt-3 flex items-center gap-1">
                      Click to create <ArrowRight className="h-3 w-3" />
                    </p>
                  </CardContent>
                </Card>
              ))}
          ...
          {tab === "history" && (
            <Card>
              <CardContent className="p-0">
                {executions.length === 0 ? (
                  <div className="py-16 text-center text-muted-foreground">No executions yet</div>
                ) : (
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b text-left">
                        <th className="py-3 px-4 font-medium">Workflow ID</th>
              ...
              <div className="grid gap-4 md:grid-cols-5">
                ...
                <Card>
                  <CardContent className="py-4">
                    ...
                    <p className="text-2xl font-bold">{analytics.avg_duration_ms}ms</p>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="py-4">
                    <p className="text-xs text-muted-foreground">Active Workflows</p>
                    <p className="text-2xl font-bold">{analytics.active_workflows}</p>
                  </CardContent>
                </Card>
              </div>
            </div>
          ...
          {tab === "builder" && (
            <div className="grid gap-6 lg:grid-cols-2">
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-2">
                    <Sparkles className="h-4 w-4 text-primary" /> Natural Language → Workflow
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <textarea
                    ...
                    rows={4}
                    placeholder="Describe your workflow... Example: When inventory falls below safety stock and supplier lead time exceeds 7 days, notify procurement and generate a draft purchase order."
                    value={aiInput}
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">Available Triggers &amp; Actions</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {[
                        ...
                        <span
                          key={t}
                          className="rounded-full bg-blue-50 dark:bg-blue-950/30 px-2 py-0.5 text-xs text-blue-700 dark:text-blue-400"
                        >
                          {t}
                        </span>
                      ))}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {[
                        ...
                        <span
                          key={a}
                          className="rounded-full bg-green-50 dark:bg-green-950/30 px-2 py-0.5 text-xs text-green-700 dark:text-green-400"
                        >
                          {a}
                        </span>
                      ))}
